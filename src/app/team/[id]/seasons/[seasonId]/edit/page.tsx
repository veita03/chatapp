...
export default function EditSeasonPage() {
  ...
  // Hydrate form when season loads (mocked or real)
  useEffect(() => {
    if (season) {
      setFormData({
        name: season.name || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/team/${teamId}`); // Redirect back to team dashboard after update
       ...
       console.error(error);
       setSubmitError(error.message || "Napaka pri urejanju sezone");
      ...
      <div className="max-w-6xl mx-auto w-full px-4 sm:px-8 mt-8 pb-12">
        <div className="ui-card p-6 md:p-10 flex flex-col">
          <div className="flex items-center space-x-3 mb-8 pb-4 border-b border-gray-100">
             <h2 className="text-[22px] font-bold text-[#353b41]" style={{fontFamily: 'var(--font-montserrat)'}}>Urejanje sezone</h2>
          </div>
