...
export default function SeasonLayoutClient({
  ...
  if (isAuthLoading || data === undefined) {
    return (
      <div className="min-h-[100dvh] flex items-center justify-center bg-[#F4F6F8]">
        <div className="w-8 h-8 rounded-full border-4 border-gray-300 border-t-[#dba032] animate-spin" />
      </div>
    );
  }
  ...
  if (data === null) {
    return (
      <div className="min-h-[100dvh] flex flex-col items-center justify-center bg-[#F4F6F8] p-4 text-center">
        <h1 className="text-2xl font-bold text-gray-800 mb-2">Sezona ne obstaja</h1>
        <p className="text-gray-500 mb-6">Povezava je napačna ali pa je bila sezona izbrisana.</p>
        <Link href="/teams" className="bg-[#dba032] text-white px-6 py-2.5 rounded-lg font-bold shadow-md hover:bg-[#c28c29] transition-colors">
          Vrnite se na Ekipe
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col md:flex-row bg-[#F4F6F8] font-sans overflow-hidden">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-[#dba032] text-white shrink-0 shadow-lg z-20">
        ...
        <div className="flex flex-col items-center mt-4 px-6 mb-8 text-center px-2">
            <div className="w-24 h-24 rounded-full bg-white/20 p-1 mb-3">
               <div className="w-full h-full rounded-full bg-white overflow-hidden flex items-center justify-center shadow-inner">
                  {team?.image ? (
                     ...
                     <span className="text-[#dba032] font-bold text-2xl">
      ...
      <div className="flex-1 flex flex-col h-[100dvh] overflow-hidden">
        ...
        <header className="h-16 md:h-20 bg-[#ecc56e] flex items-center justify-between px-4 md:px-8 shrink-0 z-10 relative">
           ...
           <div className="md:hidden flex items-center">
             <Link href="/teams">
               <span className="font-bold text-white tracking-tight">SPORT2GO</span>
             </Link>
           </div>
           
           {/* Center Context Switchers */}
           <div className="hidden md:flex flex-1 justify-center items-center gap-4">
              <button className="flex items-center gap-2 bg-white/20 hover:bg-white/30 text-white font-medium px-4 py-2 rounded-lg transition-colors text-sm max-w-[250px]">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4 shrink-0"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" /></svg>
                <div className="truncate text-left border-l border-white/20 pl-2 ml-1">
                   <span className="text-[10px] uppercase font-bold text-white/70 block leading-tight">EKIPA</span>
                   <span className="truncate block font-bold">{team?.name}</span>
                </div>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-3.5 h-3.5 shrink-0"><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg>
              </button>
              
              <button className="flex items-center gap-2 bg-[#d7931f] hover:bg-[#c2841c] text-white font-bold px-4 py-2 rounded-lg transition-colors text-sm shadow-inner max-w-[250px]">
                ...
                <div className="truncate text-left border-l border-white/20 pl-2 ml-1">
                   <span className="text-[10px] uppercase font-bold text-white/70 block leading-tight">SEZONA</span>
                   <span className="truncate block">{season?.name}</span>
                </div>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-3.5 h-3.5 shrink-0"><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg>
              </button>
           </div>
