...
export default function SeasonLayoutClient({
  ...
  useEffect(() => {
    const saved = localStorage.getItem("sport2go_sidebar_open");
    if (saved !== null) {
        ...
        <aside className={`hidden md:flex flex-col ${isSidebarOpen ? 'w-64' : 'w-[80px]'} bg-[#efc463] text-white shrink-0 overflow-y-auto z-10 pt-14 transition-all duration-300 relative hide-scrollbar`}>
              ...
              <div className={`rounded-full bg-white/20 p-1 mb-3 transition-all duration-300 ${isSidebarOpen ? 'w-24 h-24' : 'w-12 h-12'}`}>
                 <div className="w-full h-full rounded-full bg-white overflow-hidden flex items-center justify-center shadow-inner">
                    {team?.image ? (
                       ...
                       <span className={`text-[#efc463] font-bold ${isSidebarOpen ? 'text-3xl' : 'text-lg'}`}>
                         {team?.name?.charAt(0)?.toUpperCase()}
                       </span>
                    )}
                 </div>
              </div>
              {isSidebarOpen && <h3 className="font-bold text-lg leading-tight line-clamp-2 px-2" title={team?.name}>{team?.name}</h3>}
          </div>
          ...
          <nav className="flex-1 px-3 space-y-2 pb-6">
              <Link 
                ...
                className={`flex items-center gap-3 py-3 rounded-xl bg-white/20 font-semibold transition-colors shadow-sm ${isSidebarOpen ? 'px-4' : 'justify-center'}`}
                title="Sezona"
              >
                ...
                {isSidebarOpen && <span>Sezona</span>}
              </Link>
              <Link 
                href={`/season/${joinCode}/standings`}
                className={`flex items-center gap-3 py-3 rounded-xl hover:bg-white/10 font-medium transition-colors opacity-80 hover:opacity-100 ${isSidebarOpen ? 'px-4' : 'justify-center'}`}
                title="Lestvica"
              >
                ...
                {isSidebarOpen && <span>Lestvica</span>}
              </Link>
              <Link 
              ...
              >
                ...
                {isSidebarOpen && <span>Plačila</span>}
              </Link>
              <Link 
              ...
              >
                ...
                {isSidebarOpen && <span>Nastavitve</span>}
              </Link>
          </nav>
        </aside>
