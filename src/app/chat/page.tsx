...
export default function ChatPage() {
  ...
  const messages = useQuery(api.messages.get, { teamId: activeTeamId || undefined });
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-[#F4F6F8] text-[#444] font-sans overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden relative pt-2 md:pt-16">
        ...
        {/* Teams Sidebar */}
        <aside className="w-20 md:w-64 shrink-0 bg-white border-r border-[#f3ebcd] flex flex-col z-10 transition-all duration-300">
          <div className="p-3 border-b border-gray-100 hidden md:block">
            <h2 className="text-sm font-bold text-gray-800 uppercase tracking-widest pl-2">Klepeti</h2>
          </div>
          <div className="flex-1 overflow-y-auto pt-2 pb-20 no-scrollbar space-y-1 md:space-y-0.5 px-2">
            
            {/* Global Chat Button */}
            <button 
              onClick={() => setActiveTeamId(null)}
              className={`w-full flex items-center p-2 rounded-xl transition-all ${activeTeamId === null ? 'bg-[#5BA582]/10 border border-[#5BA582]/20 shadow-sm' : 'hover:bg-gray-50 border border-transparent'}`}
            >
              <div className="w-12 h-12 md:w-10 md:h-10 shrink-0 bg-gradient-to-br from-[#eeb054] to-[#e49c31] rounded-full flex items-center justify-center text-white shadow-sm overflow-hidden border border-white mx-auto md:mx-0">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5"><path d="M4.913 2.658c2.075-.27 4.19-.408 6.337-.408 2.147 0 4.262.139 6.337.408 1.922.25 3.291 1.861 3.405 3.727a4.403 4.403 0 00-1.032-.211 50.89 50.89 0 00-8.42 0c-2.358.196-4.04 2.19-4.04 4.434v4.286a4.47 4.47 0 002.433 3.984L7.28 21.53A.75.75 0 016 21v-4.03a48.527 48.527 0 01-1.087-.128C2.905 16.58 1.5 14.833 1.5 12.862V6.638c0-1.97 1.405-3.718 3.413-3.979z" /><path d="M15.75 7.5c-1.376 0-2.739.057-4.086.169C10.124 7.797 9 9.103 9 10.609v4.285c0 1.507 1.128 2.814 2.67 2.94 1.243.102 2.5.157 3.768.165l2.782 2.781a.75.75 0 001.28-.53v-2.39l.33-.026c1.542-.125 2.67-1.433 2.67-2.94v-4.286c0-1.505-1.125-2.811-2.664-2.94A49.392 49.392 0 0015.75 7.5z" /></svg>
              </div>
              <div className="hidden md:block ml-3 text-left w-full overflow-hidden">
                <span className={`block font-bold text-sm truncate ${activeTeamId === null ? 'text-[#5BA582]' : 'text-gray-700'}`}>Globalni Chat</span>
                <span className="block text-xs text-gray-400 truncate w-full">Vsi uporabniki aplikacije</span>
              </div>
            </button>

            <div className="h-px bg-gray-100 my-2 mx-4 md:mx-2" />

            {/* Teams Chat Buttons */}
            {userTeams === undefined ? (
              <div className="flex justify-center p-4"><div className="w-5 h-5 border-2 border-[#eeb054] border-t-white rounded-full animate-spin"></div></div>
            ) : userTeams && userTeams.map(team => (
              <button 
                key={team._id}
                onClick={() => setActiveTeamId(team._id)}
                className={`w-full flex items-center p-2 rounded-xl transition-all ${activeTeamId === team._id ? 'bg-[#5BA582]/10 border border-[#5BA582]/20 shadow-sm' : 'hover:bg-gray-50 border border-transparent'}`}
                title={team.name}
              >
                <div className="w-12 h-12 md:w-10 md:h-10 shrink-0 bg-[#fdfaf1] rounded-full flex items-center justify-center overflow-hidden border border-gray-200 mx-auto md:mx-0 relative">
                   {team.image ? (
                     <img src={team.image} className="w-full h-full object-cover" alt={team.name} />
                   ) : (
                     <span className="text-[#dba032] font-bold text-xs">{team.name.substring(0, 2).toUpperCase()}</span>
                   )}
                </div>
                <div className="hidden md:block ml-3 text-left w-full overflow-hidden">
                  <span className={`block font-bold text-sm truncate ${activeTeamId === team._id ? 'text-[#5BA582]' : 'text-gray-700'}`}>{team.name}</span>
                  <span className="block text-xs text-gray-400 truncate w-full">{team.memberCount} članov</span>
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="flex-1 flex flex-col relative w-full h-full pb-[0px]">
          {/* Active Chat Header */}
          <div className="absolute top-0 left-0 right-0 h-14 bg-white/80 backdrop-blur-md border-b border-gray-100 z-20 flex items-center px-4 shadow-sm">
             <div className="flex flex-col">
               <span className="font-bold text-gray-800 tracking-wide" style={{fontFamily: 'var(--font-montserrat)'}}>
                  {activeTeamId === null ? "Globalni klepet" : userTeams?.find(t => t._id === activeTeamId)?.name}
               </span>
               <span className="text-xs text-gray-400 font-medium">
                  {messages?.length || 0} sporočil
               </span>
             </div>
          </div>
          ...
          <main 
            ...
            className="flex-1 overflow-y-auto p-4 md:p-6 space-y-5 relative bg-[#F4F6F8] pt-16 pb-32"
          >
        {messages === undefined ? (
        ...
        {hasUnread && (
          <div className="absolute bottom-[200px] md:bottom-[230px] left-1/2 transform -translate-x-1/2 z-30 pointer-events-none">
            <button
        ...
        <footer className="bg-white border-t border-gray-200 flex-shrink-0 z-20 pb-safe shadow-[0_-5px_20px_-5px_rgba(0,0,0,0.04)] absolute bottom-0 right-0 left-0">
