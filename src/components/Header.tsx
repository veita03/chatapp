...
export default function Header({ onLoginClick, onRegisterClick, leftElement }: HeaderProps) {
  ...
  return (
    <header className="absolute top-0 left-0 right-0 z-50 flex flex-col md:flex-row items-center justify-between px-4 md:px-6 lg:px-8 py-3 md:py-0 gap-3 md:gap-0" style={{minHeight: '60px', background: 'linear-gradient(90deg, #eeaf53 0%, #edca78 50%, #ecdf9b 100%)'}}>
      {/* Left Side: Logo + Addons */}
      <div className="flex items-center gap-6">
        {/* Logo */}
        <a href="/" className="flex items-center space-x-2 hover:opacity-90 transition-opacity shrink-0">
          <img src="https://www.sport2go.app/image/logo.svg" alt="SPORT2GO Logo" className="h-5 md:h-6 w-auto brightness-0 invert"/>
          <span className="text-lg md:text-[22px] text-white tracking-wide leading-none drop-shadow-sm flex items-baseline select-none" style={{fontFamily: 'var(--font-montserrat)'}}>
